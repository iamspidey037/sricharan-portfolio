...
export default async function ProjectPage({ params }: Props) {
  ...
  const statusInfo = STATUS_LABELS[String(p.projectStatus ?? '')] || null;
                ...
                <div className="card p-6">
                  ...
                  {aiMetrics.modelName && (
                    <p className="text-text-secondary text-sm mb-4">
                      Model: <span className="text-primary font-mono">{String(aiMetrics.modelName)}</span>
                      {aiMetrics.framework && <> · Framework: <span className="text-secondary font-mono">{String(aiMetrics.framework)}</span></>}
                      {aiMetrics.targetHardware && <> · Hardware: <span className="font-mono">{String(aiMetrics.targetHardware)}</span></>}
                    </p>
                  )}
                  ...
                  {aiMetrics.optimizationNotes && (
                    <p className="text-xs text-text-muted mt-3 italic">{String(aiMetrics.optimizationNotes)}</p>
                  )}
                </div>
              )}
              ...
              {hardwareComponents.length > 0 && (
                <div className="card p-4">
                  <h3 className="font-heading font-semibold text-text-primary text-sm mb-3">
                    <Cpu size={13} className="inline mr-1 text-primary" /> Hardware
                  </h3>
